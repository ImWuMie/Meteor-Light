...
public class OldHitting extends Module {
        ...
        if (isOldHit(mc.player.getMainHandStack().getItem())) {
            switch (animationMode.get()) {
                ...
                case Rotate -> {
                    ...
                    rotate(matrices,MathHelper.sqrt(event.swingProgress) * 10.0F * 40.0F, 1.0F, -0.0F, 2.0F);
                }
            }
        }
    }
    ...
    public void doBlockTransformations(MatrixStack matrices) {
        matrices.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(-102f));
        matrices.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(30f));
        matrices.multiply(Vec3f.POSITIVE_Z.getDegreesQuaternion(82.5f));
    }

    public void rotate(MatrixStack matrices,float angle, float x, float y, float z) {
        matrices.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(angle*x));
        matrices.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(angle*y));
        matrices.multiply(Vec3f.POSITIVE_Z.getDegreesQuaternion(angle*z));
    }
