...
import meteordevelopment.meteorclient.systems.modules.movement.StepPlus;
import meteordevelopment.meteorclient.utils.Utils;
import meteordevelopment.meteorclient.utils.VectorUtils;
import meteordevelopment.meteorclient.utils.entity.EntityUtils;
import meteordevelopment.meteorclient.utils.misc.FindItemResultPlus;
...
public class SurroundPlus extends Module {
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (antiBed.get() && (!onlyInHole.get() || onlyInHole.get() && checkForBed(mc.player.getBlockPos()))) {
            FindItemResultPlus item = VectorUtils.findInHotbar(stack -> bedBlocks.get().contains(Block.getBlockFromItem(stack.getItem())));
    ...
    private boolean place(BlockPos pos, FindItemResultPlus item) {
        if (mc.player.getBlockPos().equals(pos) || blocksInTick >= blocksPerTick.get() || checkForPause(pos)) return false;
    ...
    private FindItemResultPlus getBestBlock() {
    ...
    private boolean place(BlockPos pos, FindItemResultPlus item, boolean rotate, boolean swingHand, boolean checkEntities) {
    ...
    private void place(BlockHitResult result, Hand hand, boolean swing, boolean sneak) {
        if (hand != null && result != null && mc.world.getWorldBorder().contains(result.getBlockPos()) && mc.player.getStackInHand(hand).getItem() instanceof BlockItem) {
            ...
            mc.getSoundManager().play(new PositionedSoundInstance(group.getPlaceSound(), SoundCategory.BLOCKS, (group.getVolume() + 1.0F) / 8.0F, group.getPitch() * 0.5F, null,result.getBlockPos()));
