...
public class Criticals extends Module {
    ...
    private int sendTimer,attacks = 0;
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (event.packet instanceof IPlayerInteractEntityC2SPacket)
            if (((IPlayerInteractEntityC2SPacket) event.packet).getType() == PlayerInteractEntityC2SPacket.InteractType.ATTACK) {
                if (skipCrit()) return;

                Entity entity =  ((IPlayerInteractEntityC2SPacket) event.packet).getEntity();
                ...
                switch (mode.get()) {
                    ...
                    case TpHop -> {
                        sendCritPacket(0.02,false);
                        sendCritPacket(0.01,false);
                        mc.player.setPos(mc.player.getX(),mc.player.getY() + 0.01, mc.player.getZ());
                    } case NCPPacket -> {
                        sendCritPacket(0.11,false);
                        sendCritPacket(0.1100013579,false);
                        sendCritPacket(0.0000013579,false);
                    } case More -> {
                        ...
                        attacks++;
                        if(attacks > 6) {
                            sendCritPacket(0.2,  false);
                            sendCritPacket(0.1216, false);
                            attacks = 0;
                        }
                    }
                    case Matrix -> {
                        attacks++;
                        if(attacks > 3) {
                            sendCritPacket(0.110314, false);
                            sendCritPacket(0.0200081, false);
                            sendCritPacket(0.00000001300009, false);
                            sendCritPacket(0.000000000022, false);
                            sendCritPacket(0,true);
                            attacks = 0;
                        }
                    }
                    default -> {
    ...
    private void sendCritPacket(double addY, boolean ground) {
        assert mc.player != null;
        double x = mc.player.getX();
        double y = mc.player.getY() + addY;
