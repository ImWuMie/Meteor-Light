...
import meteordevelopment.meteorclient.utils.render.ScreenUtil;
...
public class ActiveModulesHud extends HudElement {
    ...
    public void render(HudRenderer renderer) {
        ...
        switch (renderMode.get()) {
            case Jello -> {
                ...
                ScreenUtil sr = new ScreenUtil(mc);
                ...
                for (Module module : modules) {
                    if (module.isActive()) {
                        TTFFontRender font = CFont.jelloLight;
                        double ooffset = alignX(getModuleWidth(font, module,0.75), alignment.get());
                        xStart = (float) ((float) x + ooffset);
                        renderer.texture(new MeteorIdentifier("textures/arraylistshadow.png"),(xStart - 8 - 2 - 1), (yStart + 2 - 2.5f - 1.5f - 1.5f - 1.5f - 6 - 1), (font.getWidth(module.title, false,0.75) * 1 + 20 + 10),  (18.5 + 6 + 12 + 2),Color.WHITE);
                        GlStateManager._enableBlend();
                        jelloFont.render(module.title,xStart,yStart+7.5f,new Color(0xffffffff),false,0.75);
                        yStart += 7.5f + 5.25f; // 15 + 10.5
                    }
                }
            }
            case Meteor -> {
    ...
    private double getModuleWidth(TTFFontRender jelloFont, Module module,double scale) {
        double width = jelloFont.getWidth(module.title,false,scale);
