...
package meteordevelopment.meteorclient.glrender.opengl;
...
import meteordevelopment.meteorclient.glrender.Image;
import meteordevelopment.meteorclient.glrender.Paint;
import meteordevelopment.meteorclient.glrender.font.GLBakedGlyph;
import meteordevelopment.meteorclient.glrender.font.GLFontAtlas;
import meteordevelopment.meteorclient.glrender.font.LayoutCache;
import meteordevelopment.meteorclient.glrender.font.LayoutPiece;
import meteordevelopment.meteorclient.glrender.math.FMath;
import meteordevelopment.meteorclient.glrender.math.Matrix4;
import meteordevelopment.meteorclient.glrender.math.Rect;
import meteordevelopment.meteorclient.glrender.math.RectF;
...
import static meteordevelopment.meteorclient.MeteorClient.LOGGER;
...
public final class GLSurfaceCanvas extends GLCanvas {
    private static String ID = "meteor-client";
    ...
    public GLSurfaceCanvas() {
        ...
        LOGGER.info(MARKER, "Created OpenGL surface canvas");
    }
    ...
    private static void onLoadShaders(@Nonnull ShaderManager manager) {
        int posColor = manager.getShard(ID, "pos_color.vert");
        int posColorTex = manager.getShard(ID, "pos_color_tex.vert");
        int posTex = manager.getShard(ID, "pos_tex.vert");

        int colorFill = manager.getShard(ID, "color_fill.frag");
        int colorTex = manager.getShard(ID, "color_tex.frag");
        int roundRectFill = manager.getShard(ID, "round_rect_fill.frag");
        int roundRectTex = manager.getShard(ID, "round_rect_tex.frag");
        int roundRectStroke = manager.getShard(ID, "round_rect_stroke.frag");
        int circleFill = manager.getShard(ID, "circle_fill.frag");
        int circleStroke = manager.getShard(ID, "circle_stroke.frag");
        int arcFill = manager.getShard(ID, "arc_fill.frag");
        int arcStroke = manager.getShard(ID, "arc_stroke.frag");
        int quadBezier = manager.getShard(ID, "quadratic_bezier.frag");
        int alphaTex = manager.getShard(ID, "alpha_tex.frag");
        int colorTexMs = manager.getShard(ID, "color_tex_4x.frag");
        int glowWave = manager.getShard(ID, "glow_wave.frag");
        ...
        LOGGER.info(MARKER, "Loaded OpenGL canvas shaders");
    }
    ...
    private ByteBuffer checkPosColorMemory() {
        if (mPosColorMemory.remaining() < 48) {
            ...
            LOGGER.debug(MARKER, "Grow pos color buffer to {} bytes", newCap);
        }
    ...
    private ByteBuffer checkPosColorTexMemory() {
        if (mPosColorTexMemory.remaining() < POS_COLOR_TEX_VERTEX_SIZE * 4) {
            ...
            LOGGER.debug(MARKER, "Grow pos color tex buffer to {} bytes", newCap);
        }
    ...
    /*private ByteBuffer getModelViewBuffer() {
        if (mModelViewData.remaining() < 64) {
            ...
            LOGGER.debug(MARKER, "Resize model view buffer to {} bytes", mModelViewData.capacity());
        }
    ...
    private ByteBuffer checkPosTexMemory() {
        if (mGlyphMemory.remaining() < 64) {
            ...
            mGlyphResized = true;
            LOGGER.debug(MARKER, "Grow pos tex buffer to {} bytes", newCap);
        }
        return mGlyphMemory;
    }
    ...
    private ByteBuffer checkUniformMemory() {
        if (mUniformMemory.remaining() < 64) {
            ...
            LOGGER.debug(MARKER, "Grow general uniform buffer to {} bytes", newCap);
        }
    ...
    public void drawLayer(@Nonnull GLTexture texture, float w, float h, float alpha, boolean flipY) {
        ...
        if (target == GL_TEXTURE_2D || target == GL_TEXTURE_2D_MULTISAMPLE) {
            ...
            LOGGER.warn(MARKER, "Cannot draw texture target {}", target);
        }
    }
    ...
    @Override
    public void drawText(@Nonnull CharSequence text, int start, int end, float x, float y, int align, @Nonnull TextPaint paint) {
