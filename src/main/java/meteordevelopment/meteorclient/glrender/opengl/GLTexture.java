...
public class GLTexture extends GLObject {
    ...
    public void allocate2D(int internalFormat, int width, int height, int maxLevel) {
        ...
        if (dimension != 0 && dimension != dim) {
            LOGGER.warn(GLCore.MARKER, "Inconsistent 2D texture dimension, set 0x{} but allocated 0x{}",
    ...
    public void allocate2DCompat(int internalFormat, int width, int height, int maxLevel) {
        ...
        dim |= 1L << DEPTH_SHIFT;
        if (dimension != 0 && dimension != dim) {
            LOGGER.warn(GLCore.MARKER, "Inconsistent 2D(M) texture dimension, set 0x{} but allocated 0x{}",
                    Long.toHexString(dimension), Long.toHexString(dim));
        }
        dimension = dim;
    ...
    public void allocate2DMS(int internalFormat, int width, int height, int samples) {
        ...
        dim |= 1L << DEPTH_SHIFT;
        if (dimension != 0 && dimension != dim) {
            LOGGER.warn(GLCore.MARKER, "Inconsistent 2DMS texture dimension, set 0x{} but allocated 0x{}",
                    Long.toHexString(dimension), Long.toHexString(dim));
        }
        dimension = dim;
    }
