...
import meteordevelopment.meteorclient.glrender.R;
import meteordevelopment.meteorclient.glrender.Window;
import meteordevelopment.meteorclient.glrender.drawable.Drawable;
import meteordevelopment.meteorclient.glrender.drawable.StateListDrawable;
import meteordevelopment.meteorclient.glrender.math.Rect;
import meteordevelopment.meteorclient.glrender.transition.Transition;
import meteordevelopment.meteorclient.glrender.transition.TransitionListener;
import meteordevelopment.meteorclient.glrender.transition.TransitionManager;
...
import static meteordevelopment.meteorclient.glrender.view.ViewGroup.LayoutParams.MATCH_PARENT;
import static meteordevelopment.meteorclient.glrender.view.ViewGroup.LayoutParams.WRAP_CONTENT;
...
public class PopupWindow {
     ...
     */
    private void invokePopup(@Nonnull ViewGroup.LayoutParams p) {
        final DecorView decorView = mDecorView;

        setLayoutDirectionFromAnchor();

        MeteorGL.getInstance().getViewManager().addView(decorView, p);
    ...
    private void dismissImmediate(@Nonnull View decorView, @Nullable ViewGroup contentHolder, View contentView) {
        ...
        if (decorView.getParent() != null) {
            MeteorGL.getInstance().getViewManager().removeView(decorView);
        }
    ...
    protected void update(View anchor, ViewGroup.LayoutParams params) {
        setLayoutDirectionFromAnchor();
        MeteorGL.getInstance().getViewManager().updateViewLayout(mDecorView, params);
    }
    ...
    private class DecorView extends FrameLayout implements CoordinatorLayout.AttachedBehavior {
        ...
        private final CoordinatorLayout.Behavior<? extends DecorView> mBehavior;
        ...
        public DecorView(@Nonnull CoordinatorLayout.Behavior<? extends DecorView> behavior) {
            mBehavior = behavior;
        }

        @Nonnull
        @Override
        public CoordinatorLayout.Behavior<? extends DecorView> getBehavior() {
            return mBehavior;
        }
