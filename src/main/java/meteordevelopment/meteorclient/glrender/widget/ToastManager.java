...
public final class ToastManager {
    ...
    private void showNextToastLocked() {
        ...
        MeteorGL.getInstance().getViewManager().addView(mTextView, mParams);
    ...
    private void cancelToastLocked(@Nonnull ToastRecord record) {
        MeteorGL.getInstance().getViewManager().removeView(mTextView);
        ...
        synchronized (mToastQueue) {
            ...
            if (record != null) {
                ...
                if (mToastQueue.size() >= MAX_TOASTS) {
                    MeteorClient.LOGGER.error(MARKER, "System has already queued {} toasts. Not showing more.",
                            mToastQueue.size());
                    return;
                }
    ...
    public void cancelToast(@Nonnull Toast token) {
        synchronized (mToastQueue) {
            ...
            if (r != null) {
                ...
                MeteorClient.LOGGER.warn(MARKER, "Toast already cancelled. token={}", token);
            }
        }
    }
