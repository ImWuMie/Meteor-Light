...
public final class Choreographer {
    ...
    private void postCallbackDelayedInternal(int callbackType, @Nonnull Object action, @Nullable Object token,
                                             long delayMillis) {
        if (DEBUG_FRAMES) {
            MeteorGL.LOGGER.info(MARKER, "PostCallback: type=" + callbackType
    ...
    private void removeCallbacksInternal(int callbackType, @Nullable Object action, @Nullable Object token) {
        if (DEBUG_FRAMES) {
           MeteorGL.LOGGER.info(MARKER, "RemoveCallbacks: type=" + callbackType
                    + ", action=" + action + ", token=" + token);
        }
    ...
    public void scheduleFrameAsync(long timestampNanos) {
        ...
        synchronized (mLock) {
            ...
            if (DEBUG_FRAMES) {
                MeteorGL.LOGGER.info(MARKER, "Scheduling next frame in " + (timestampNanos - now) / 1000000 + " ms.");
            }
    ...
    void doFrame() {
        ...
        if (DEBUG_FRAMES) {
            ...
            MeteorGL.LOGGER.info(MARKER, "Frame : Finished, took "
    ...
    void doCallbacks(int callbackType, long frameTimeNanos) {
        ...
        try {
            for (CallbackRecord c = callbacks; c != null; c = c.next) {
                if (DEBUG_FRAMES) {
                    MeteorGL.LOGGER.info(MARKER, "RunCallback: type=" + callbackType
