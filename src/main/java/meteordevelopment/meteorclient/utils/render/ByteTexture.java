...
public class ByteTexture extends AbstractTexture {
    ...
    public ByteTexture(BufferedImage bufferedImage,ByteBuffer buffer,Format format, Filter filterMin, Filter filterMag) {
        if (!RenderSystem.isOnRenderThread()) {
            RenderSystem.recordRenderCall(() -> upload(bufferedImage.getWidth(), bufferedImage.getHeight(), buffer, format, filterMin, filterMag));
        } else {
            upload(bufferedImage.getWidth(), bufferedImage.getHeight(), buffer, format, filterMin, filterMag);
        }
    }
