...
public class MineplexTpAuraNodeProcessor extends NodeProcessor {
	...
	public Node createNode(BlockPos pos) {
		Node node = new Node(!isStairBlock(Utils.getBlock(pos.down())) && isPassable(Utils.getBlockState(pos),pos) && isPassable(Utils.getBlockState(pos.up()),pos), pos).setId(pos.hashCode());
		return node;
	}
	...
	public boolean isWalkable(BlockState block,BlockPos pos) {
		return !(block.getMaterial() == Material.AIR) && (block.getBlock() instanceof LilyPadBlock || block.getBlock() instanceof VineBlock || block.getBlock() instanceof LadderBlock || block.isFullCube(MinecraftClient.getInstance().world, pos));
	}
